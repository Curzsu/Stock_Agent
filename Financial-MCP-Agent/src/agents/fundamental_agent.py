...
# 4. 准备输入数据，构建详细的分析请求
            ...
            if not stock_code and not company_name:
                logger.error(f"{ERROR_ICON} FundamentalAgent: No stock code or company name provided.")
                current_data["fundamental_analysis_error"] = "No stock code or company name provided."
                current_data["fundamental_analysis"] = "无法进行基本面分析：未提供股票代码或公司名称。请提供正确的股票名称或代码进行分析。"
                execution_logger.log_agent_complete(agent_name, current_data, time.time() - agent_start_time, False, "No stock info")
                return {"data": current_data, "messages": current_messages, "metadata": current_metadata}
            
            if stock_code == 'Unknown' or not stock_code:
                stock_code = None
            ...
            if not stock_code and not company_name:
                logger.error(f"{ERROR_ICON} FundamentalAgent: No valid stock code or company name provided.")
                current_data["fundamental_analysis_error"] = "No valid stock code or company name provided."
                current_data["fundamental_analysis"] = "无法进行基本面分析：未提供有效的股票代码或公司名称。请提供正确的股票名称或代码进行分析。"
                execution_logger.log_agent_complete(agent_name, current_data, time.time() - agent_start_time, False, "No valid stock info")
                return {"data": current_data, "messages": current_messages, "metadata": current_metadata}
            
            if stock_code:
                agent_input = f"""请分析{company_name or '该公司'}（股票代码：{stock_code}）的基本面情况。
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的时间周期或其他工具组合，基于可用信息提供尽可能全面的分析。"""
                ...
                agent_input = f"""请分析{company_name}的基本面情况。

当前时间：{current_time_info}
当前日期：{current_date}

警告：股票代码当前未知。请首先使用 `search_stock_by_name` 工具搜索 "{company_name}" 的股票代码。
一旦找到正确的股票代码（例如 sh.600000 或 sz.000001），请继续进行以下基本面分析：
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的时间周期或其他工具组合，基于可用信息提供尽可能全面的分析。"""
            ...
            # 调用 Agent执行分析，设置递归限制以防止死循环
            try:
                # 设置300秒（5分钟）超时
                response = await asyncio.wait_for(
                    agent.ainvoke(input_data, config={"recursion_limit": 15}),
                    timeout=300
                )
            except asyncio.TimeoutError:
                logger.error("ReAct agent execution timed out")
                response = {
                    "messages": [AIMessage(content="分析过程超时（超过5分钟）。可能原因：网络连接慢，或无法找到有效数据。")]
                }
            except Exception as e:
                logger.error(f"ReAct agent execution failed: {e}")
                response = {
                    "messages": [AIMessage(content=f"分析过程失败。错误信息: {str(e)}")]
                }
