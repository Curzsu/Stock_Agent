    ...
    try:
        ...
        try:
            ...
            if not stock_code and not company_name:
                logger.error(f"{ERROR_ICON} NewsAgent: No stock code or company name provided.")
                current_data["news_analysis_error"] = "No stock code or company name provided."
                current_data["news_analysis"] = "无法进行新闻分析：未提供股票代码或公司名称。请提供正确的股票名称或代码进行分析。"
            ...
            if not stock_code and not company_name:
                logger.error(f"{ERROR_ICON} NewsAgent: No valid stock code or company name provided.")
                current_data["news_analysis_error"] = "No valid stock code or company name provided."
                current_data["news_analysis"] = "无法进行新闻分析：未提供有效的股票代码或公司名称。请提供正确的股票名称或代码进行分析。"
                execution_logger.log_agent_complete(agent_name, current_data, time.time() - agent_start_time, False, "No valid stock info")
                return {"data": current_data, "messages": current_messages, "metadata": current_metadata}
            
            if stock_code:
                agent_input = f"""请对{company_name or '该公司'}（股票代码：{stock_code}）进行新闻分析。

当前时间：{current_time_info}
当前日期：{current_date}

请进行以下新闻分析：
1. 爬取与{company_name or '该公司'}相关的最新新闻（至少5条）
...
请使用可用的工具获取实际新闻数据进行分析，确保情感分析和风险评估的准确性。如果某些新闻无法获取，请基于可用信息提供尽可能全面的分析。"""
                ...
                agent_input = f"""请对{company_name}进行新闻分析。

当前时间：{current_time_info}
当前日期：{current_date}

警告：股票代码当前未知。请首先使用 `search_stock_by_name` 工具搜索 "{company_name}" 的股票代码。
一旦找到正确的股票代码（例如 sh.600000 或 sz.000001），请继续进行以下新闻分析：
