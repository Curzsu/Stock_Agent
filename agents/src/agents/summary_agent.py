...
def truncate_report_at_baseline_time(report_content: str, current_time_info: str) -> str:
    ...
    注意：此函数只在报告末尾附近查找时间信息，避免误截断开头的标题
    ...
    Returns:
        截断后的报告内容
    """
    # 只在报告末尾20%的内容中查找，避免误截断开头的标题
    search_start = int(len(report_content) * 0.8)
    search_region = report_content[search_start:]

    # 构建多种可能的"分析基准时间"模式
    baseline_patterns = [
        rf'分析基准时间[：:]\s*{re.escape(current_time_info)}',
        rf'分析基准日期[：:]\s*{re.escape(current_time_info.split()[0]) if current_time_info.split() else current_time_info}',
    ...
    for pattern in baseline_patterns:
        ...
        if match:
            ...
            # 查找该行的结束位置（换行符）
            line_end = search_region.find('\n', end_pos)
            if line_end == -1:
                # 如果没有换行符，说明是最后一行，直接截断
                truncated_content = report_content[:search_start + end_pos].strip()
            else:
                # 截断到该行结束
                truncated_content = report_content[:search_start + line_end].strip()

            logger.info(f"截断报告在末尾'分析基准时间'行之后")
            return truncated_content

    # 如果没有找到匹配的模式，返回原始内容
    logger.info("未在报告末尾找到'分析基准时间'模式，返回完整报告内容")
