...
**重要限制：**
1. 请专注于价格数据和技术指标分析，不要使用crawl_news工具获取新闻信息。技术分析应该基于价格走势、成交量和技术指标数据，而不是新闻事件。
2. **获取K线数据时，必须使用不复权价格（adjust_flag="3"），不要使用前复权或后复权价格。报告中的股价必须是真实的交易价格，而非复权调整后的价格。**
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。"""
            ...
            try:
                ...
                print(f"TECHNICALAGENT: [Output contains emoji, length: {len(final_output)} chars]")
            # 7. 记录LLM交互，用于后续分析和优化
            model_config = {
                "model": model_name,
                "temperature": 0.3,
