...
**重要限制：**
1. 请专注于估值指标和财务数据分析，不要使用crawl_news工具获取新闻信息。估值分析应该基于财务指标、估值比率和历史数据，而不是新闻事件。
2. **获取股价或K线数据时，必须使用不复权价格（adjust_flag="3"）。报告中的股价必须是真实的交易价格，以便正确计算市盈率、市净率等估值指标。**
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的工具或参数组合，基于可用信息提供尽可能全面的分析。请保持回答简洁，避免冗长的描述性文字"""
            ...
            try:
                ...
                print(f"VALUEAGENT: [Output contains emoji, length: {len(final_output)} chars]")
            # 7. 记录LLM交互，用于后续分析和优化
            model_config = {
                "model": model_name,
                "temperature": 0.3,
